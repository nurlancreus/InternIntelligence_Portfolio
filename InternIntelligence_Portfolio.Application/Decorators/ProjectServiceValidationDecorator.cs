...
namespace InternIntelligence_Portfolio.Application.Decorators
{
    public class ProjectServiceValidationDecorator(IProjectService innerProjectService, RequestValidator requestValidator) : IProjectService
    {
        ...
        public async Task<Result<Guid>> UpdateAsync(Guid id, UpdateProjectRequestDTO request, CancellationToken cancellationToken = default)
        {
            var validationResult = await _requestValidator.ValidateAsync(request, cancellationToken);
            ...
            return await _innerProjectService.UpdateAsync(id, request, cancellationToken);
        }
    }
}
